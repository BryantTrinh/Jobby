...
def parseWebsite(url):
    ...
    if urlComponents.netloc in websiteSet:
        if urlComponents.netloc == 'www.indeed.com':
            parseIndeed(url, queryDict, soup)
        elif urlComponents.netloc == 'www.linkedin.com':
            parseLinkedIn(url, queryDict)
    # else:
    #     Do other parsing


def parseIndeed(url, queryDict, soup):
    # print(jobData)
    htmlText = None
    if 'vjk' in queryDict:
        # parse from jobList
...
def parseLinkedIn(url, queryDict):
