...
from .scrapeUtils import linkToSoup_selenium, parseJson, linkToSoup_selenium_linkedIn
...
def parse_website(url):
    ...
    if urlComponents.netloc in websiteSet:
        if urlComponents.netloc == 'www.indeed.com':
            ...
            soup = linkToSoup_selenium_linkedIn(url, tmout = 8, isv=True, returnErr=False)
            if not soup:
                print('Error getting soup')
                return
            print(soup)
            # jobDict = parse_linkedIn(url, queryDict, soup)
    # else:
    #     Do other parsing
    return jobDict
...
def parse_linkedIn(url, queryDict, soup):
    print('parse linkedin site')

    # print(queryDict)

    jobData = soup.select('.details')
    # jobData = soup.select('.job-view-layout.jobs-details')
